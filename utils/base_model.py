...
class BaseModelLogit:
    ...
    def logit_summary(self):
        ...
        data_summary.sort_values(by=['Odds'], ascending=False).reset_index(
            drop=True)
    ...
    def confusion_matrix(self):
        ...
        mat = self.model_fit.pred_table(threshold=0.5)
        # logger.info('{} : {} : {}'.format(
        #     mat[1][1], mat[ :,1].sum(), mat[1:,].sum())
        #                             )
        self.cm_df = pd.DataFrame(mat)
        self.cm_df = self.cm_df.astype(int)
        ...
        self.metric['F1_score'] = \
            round(2*(self.metric['Precision']*self.metric['Recall']) /
                  (self.metric['Precision'] + self.metric['Recall']), 4)
...
class BaseDecisionTree:
    ...
    def classfier(self):
        self.model_fit = DecisionTreeClassifier(
            criterion='gini',
            splitter='best',
            max_depth=None,
            min_samples_split=2,
            ...
            class_weight=None,
            ccp_alpha=0.0).fit(self.X, self.Y)


class BaseGradiantDescentBinaryClassifier:
    """  """
    def __init__(self, X, Y,) -> None:
        self.X = X
        self.Y = Y
